...
LL calc(LL p, LL q) {
    ...
    REP(i, N) {
        REP(j, N) if (i != j) {
            const auto np = xy[i].first - xy[j].first;
            const auto nq = xy[i].second - xy[j].second;

            np == p && nq == q && uf.unite(i, j);
        }
    }

    return uf.group_size();
}
...
int main(void) {
    ...
    REP(i, N) {
        REP(j, N) if (i != j) {
            const auto p = xy[i].first - xy[j].first;
            const auto q = xy[i].second - xy[j].second;
            chmin(ans, calc(p, q));
        }
    }
