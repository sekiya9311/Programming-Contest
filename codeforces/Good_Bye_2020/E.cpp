...
void naive() {
    ...
    REP(i, N) REP(j, N) REP(k, N) {
        const auto l = (x[i] & x[j]) % MOD;
        const auto r = (x[j] | x[k]) % MOD;
        (ans += (l * r) % MOD) %= MOD;
    }
    printf("NAIVE: %lld\n", ans);
}
...
void solve2() {
    ...
    LL ans = 0;
    REP(j, N) {
        LL cur_and = 0, cur_or = 0;
        REP(bit, 60) {
            const auto v = (1LL << bit) % MOD;
            if (x[j] & (1LL << bit)) {
                (cur_or += v * N % MOD) %= MOD;
                (cur_and += v * cnt[bit] % MOD) %= MOD;
            } else {
                (cur_or += v * cnt[bit] % MOD) %= MOD;
            }
        }
        const auto cur_add = cur_and * cur_or % MOD;
        (ans += cur_add) %= MOD;
    }
