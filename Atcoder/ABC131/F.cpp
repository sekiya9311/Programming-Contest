...
int N;
int x[MAX], y[MAX];
unordered_set<int> x_to_y[MAX], y_to_x[MAX];
union_find<> uf_x(MAX), uf_y(MAX);

int main(void) {
    scanf("%d", &N);
    REP(i, N) {
        scanf("%d%d", x + i, y + i);
        x_to_y[x[i]].emplace(y[i]);
        y_to_x[y[i]].emplace(x[i]);
    }
    ...
    LL ans = 0;
    REP(i, MAX) {
        if (uf_x.find(i) != i) continue;
        if (x_to_y[i].size() == 0) continue;

        ans += 1ll * uf_x.size(i) * uf_y.size(*begin(x_to_y[i]));
    }
