...
int main(void) {
  ...
  union_find<> uf(N);
  REP(i, M) {
    while (nxt_con < N - 1 && edges[nxt_con].first <= q[i].first) {
      const int u = edges[nxt_con].second.first;
      const int v = edges[nxt_con].second.second;
      nxt_con++;
      if (uf.same(u, v)) continue;

      const LL cur_u_size = uf.size(u);
      const LL cur_v_size = uf.size(v);
      prev_ans -= cur_u_size * (cur_u_size - 1) / 2;
      prev_ans -= cur_v_size * (cur_v_size - 1) / 2;

      uf.unite(u, v);
      const LL cur_uv_size = uf.size(u);
      prev_ans += cur_uv_size * (cur_uv_size - 1) / 2;
    }
    ans[q[i].second] = prev_ans;
  }

  REP(i, M) {
    printf("%lld%c", ans[i], (i + 1 == M ? '\n' : ' '));
  }
}
