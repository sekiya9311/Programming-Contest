...
bool check(LL mid) {
    VL v;
    REP(i, C) {
        LL scan = max(mid - P[i], 0ll);
        v.EB(min(scan / S[i], M[i]));
    }
    RSORT(v);
    return accumulate(begin(v), begin(v) + R, 0ll) >= B; // オーバーフロー？💢
}

void solve(int testNum) {
    scanf("%lld%lld%lld", &R, &B, &C);
    REP(i, C) scanf("%lld%lld%lld", M + i, S + i, P + i);
    LL ans = 0;
    LL high = 1e18, low = 0;    // <--- 上限値💢
    while (high - low > 5) {
